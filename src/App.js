...
function App() {
  ...
  useEffect(()=>{
    const getItems = async () => {
      const todos = await axios.get('http://localhost:5500/items')
  ...
  const addItems = async () => {
    const todos = await axios.post('http://localhost:5500/items',
      {
        item: inputVal
      }
    )
  ...
  const updateItem = async() => {
    await axios.put('http://localhost:5500/items/' + isUpdating, {item:updateItemText})
